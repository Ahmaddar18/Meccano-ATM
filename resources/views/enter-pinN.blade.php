            	...
            	<div id="wizard">
					<div align="center">
						...
						<div align="center" vertical-align="center">
								...
								<a href="/homeN/"><button class="button button2" aria-label="Continue">Continue</button></a>
								</div>
						</div>
					</div>
				</div>
...
$( "#PINcode" ).html(
	"<form action='' method='' name='PINform' id='PINform' autocomplete='off' draggable='true'>" +
		...
		"<br>" +
		"<input type='button' class='PINbutton clear' name='-' value='klar' id='-' onClick=clearForm(this); />" +
		"<input type='button' class='PINbutton' name='0' value='0' id='0' onClick=addNumber(this); />" +
		"<input type='button' class='PINbutton enter' name='+' value='Tast inn' id='+' onClick=submitForm(PINbox); />" +
	"</form>"
);
...
function submitForm(e) {
	if (e.value == "") {
		alert("Skriv inn en PIN-kode");
	}else if (e.value.length <=3){
        alert("Pin kan ikke være mindre enn 4 tegn");
	} else {
		data = {
			pin: e.value
		}
		window.location.href = "/withdraw-cashN/";
		$( "#PINbox" ).val( "" );
	};
};
</script>
